...
#import "../../libs/tuileur/patterns.typ": *
...
#pop.title-box(
  ...
  logos: (
    ...
    box(
      image("../../media/logos/utokyo.svg", width: 22%),
    ),
    image("../../media/logos/jsps.svg", width: 15%),
  ),
  logo-gap: 5em,
  banner-height: 16%,
  tiler-fn: tiler-matrix,
)
...
#let tweet(
  ...
  #grid(
    ...
    // top-right logo
    align(right + top)[
      #image("components/bsky.svg", width: 30%)
    ],
  )
...
#columns(
  ...
  [
    ...
    #pop.column-box(heading: "1. Introduction")[
      - Le lexème « *perso* » est une forme apocopée attestée dans plusieurs dictionnaires en ligne (Wiktionnaire, Larousse, Le Robert, Antidote, TLFi), mais ses trois catégories possibles ne sont pas toujours répertoriées. Il peut être l'apocope :
        - du nom commun « personnage » (Wiktionnaire) ;
        #v(-0.8em)
        #quote[Le *perso* principal de ce roman est ennuyant.]
        - de l'adjectif épithète « personnel » (tous les dictionnaires susnommés) ;
        #v(-0.8em)
        #quote[C'est mon adresse e-mail *perso*.]
        - de l'adverbe « personnellement » (Wiktionnaire, Le Robert).
        #v(-0.8em)
        #quote[*Perso*, il ne m'a pas convaincu.]
      ...
      - En raison de son appartenance au registre informel, nous nous appuyons sur le discours numérique@Paveau2017, plus particulièrement celui des réseaux sociaux, pour comprendre et analyser cet usage.
    ]

    #pop.column-box(heading: "2. Méthodologie")[
      - Collecte automatique de publications françaises contenant le lexème « perso » sur le réseau social BlueSky (alternative à X) via son API par la requête `"perso" lang:fr`.
    ...
    Selon *Molinier (2003, pp. 361-363)*, « personnnellement » peut difficilement être combiné à un pronom autre qu'un pronom de 1ʳᵉ personne :
    ...
    #pop.column-box()[
      ...
      ))[Comme j'ai dit à une autre : ➡️Les médias desinforment, mesinforment. ➡️Les gens qui aiment pas FI repostent et vont de leur ptit commentaire ➡️D'autres en comprennent ce qu' iels veulent et répètent avec leurs mots Et on arrive à " FI est antivaxx ". Et *perso*, dans cette histoire, y a pas qu’en Ehpad que les soignants doivent se vacciner. ]
    ]
...
//     column-gutter: 3em,
//     //[For more details, see Schubert _et al._, npj Computational Materials (2024) \ enzo.doyen\@unistra.fr — www.edoyen.com],
//     align(
//       horizon + left,
//     )[Read our full paper for more details: \ #text("Doyen, E. & Todirascu, A. (2025).", weight: 0) #text("Man Made Language Models? Evaluating LLMs’ Perpetuation of Masculine Generics Bias", weight: 0, style: "italic")#text(". arXiv: 2502.10577.", weight: 0)],
//     move(grid.cell(image("../figures/mg/qr.svg", height: 5em)), dx: 1em),
//     move(grid.cell(image("../figures/mg/alps.png", height: 3em)), dx: 2em),
//     move(
