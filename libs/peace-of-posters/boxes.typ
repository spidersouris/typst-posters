...
#let gen-label-right(box-counter) = label(
  "COLUMN-BOX-RIGHT-" + str(box-counter),
)
...
#let stretch-box-to-next(
  ...
  let target = b1
    ...
    .map(b => {
      ...
      let (dist, dist-without-spacing) = _calculate-vertical-distance(
        m-loc,
        c-box,
        spacing,
      )
...
#let common-box(
  ...
  if heading-box-args == none {
    ...
    if body != none {
      heading-box-args = pt.at(
        "heading-box-args-with-body",
        default: heading-box-args,
      )
    }
  }
  ...
  if heading-text-args == none {
    ...
    if body != none {
      heading-text-args = pt.at(
        "heading-text-args-with-body",
        default: heading-text-args,
      )
    }
  }
  ...
  if heading != none {
    heading-box = heading-box-function(..heading-box-args)[#heading]
  }
  ...
  if body-box-args == none {
    ...
    if heading == none {
      body-box-args = pt.at(
        "body-box-args-with-heading",
        default: body-box-args,
      )
    }
  }
  ...
  if body-text-args == none {
    ...
    if heading == none {
      body-text-args = pt.at(
        "body-text-args-with-heading",
        default: body-text-args,
      )
    }
  }
...
#let title-box(
  ...
  context {
    ...
    let title-fill = if title-fill == none { white } else { title-fill }

    /// Generate body of box
    let text-content = [
      #set text(weight: "bold", size: title-size, fill: title-fill)
      #title\
      #set text(size: subtitle-size)
      #if subtitle != none { [#subtitle\ ] }
      #if authors != none { text(weight: "bold", size: authors-size, authors) }
    ]

    if title-background != none {
      let title-background-outset = if title-background-outset == none { 0.5em } else { title-background-outset }
      text-content = box(text-content, fill: title-background, outset: title-background-outset)
    }

    /// Finally construct the main rectangle
    let indent = 2cm
    let bg-box-content = none
    if tiler-fn != none {
      bg-box-content = if type(tiler-fn) == function {
        tiler-fn(image_options: (width: 100% + 2 * indent, height: banner-height))
      } else {
        tiler-fn
      }
    } else if background-image != none {
      bg-box-content = image(background-image, width: 100% + 2 * indent, height: banner-height)
    } else {
      panic("Cannot create bg-box-content")
    }
    ...
    v(indent - banner-height - 1em)

    if type(logos) == array {
      if logos.len() > 3 { panic("Too many logos provided. Maximum is 3.") }
...
#let bottom-box(body, text-relative-width: 70%, logo: none, ..args) = {
  let body = [
    ...
    #set align(top + left)
      ...
      stack(dir: ltr, box(width: text-relative-width, body), align(
        right + horizon,
        logo,
      ))
    }
  ...
  context {
    let pt = _state-poster-theme.at(here())
    let r = common-box(
      body: box(inset: 0.7em, body),
      bottom-box: true,
      body-box-args: pt.at("bottom-box-args"),
      body-text-args: pt.at("bottom-text-args"),
      ..args,
    )
    align(bottom, r)
  }
